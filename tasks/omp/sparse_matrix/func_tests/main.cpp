...
TEST(sparse_matrix_multiplication_omp, test_square_matrices) {
  ...
  sparse_matrix_multiplication_omp::CCSMatrixOMP multiplicationTask(taskData);
...
TEST(sparse_matrix_multiplication_omp, test_rectangular_matrices) {
  const auto epsilon = 1e-6;
...
TEST(sparse_matrix_multiplication_omp, test_acceptable_sizes) {
  std::vector<double> matrixA{0, 1, 0, 6, 0, 0, 4, 3, 1, 0, 0, 2};
  std::vector<double> matrixB{0.5, 0, 1.5, 0, 0, 8.0, 3.0, 0, 0, 7, 0, 2};
...
TEST(sparse_matrix_multiplication_omp, test_empty_matrices_multiplication) {
...
TEST(sparse_matrix_multiplication_omp, test_random_square_matrices_multiplication) {
  const auto epsilon = 1e-6;
  const auto size = 50;

  auto matrixA = sparse_matrix_multiplication_omp::GenerateRandomMatrix(size * size);
  auto matrixB = sparse_matrix_multiplication_omp::GenerateRandomMatrix(size * size);
  ...
  auto expectedOutput = sparse_matrix_multiplication_omp::MultiplyMatrices(matrixA, size, size, matrixB, size, size);
...
TEST(sparse_matrix_multiplication_omp, test_random_large_matrices_multiplication) {
  const auto epsilon = 1e-6;

  auto matrixA = sparse_matrix_multiplication_omp::GenerateRandomMatrix(240);
  auto matrixB = sparse_matrix_multiplication_omp::GenerateRandomMatrix(240);
  ...
  auto expectedOutput = sparse_matrix_multiplication_omp::MultiplyMatrices(matrixA, 12, 20, matrixB, 20, 12);
...
TEST(sparse_matrix_multiplication_omp, test_matrices_200) {
  const auto size = 200;
  ...
  sparse_matrix_multiplication_omp::CCSMatrixOMP multiplicationTask(task_data_omp);
  ASSERT_TRUE(multiplicationTask.Validation()) << "Validation failed for random large matrices!";
...
TEST(sparse_matrix_multiplication_omp, test_matrices_300) {
  const auto size = 300;
...
TEST(sparse_matrix_multiplication_omp, test_matrices_400) {
  const auto size = 400;
