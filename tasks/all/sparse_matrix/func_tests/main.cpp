...
TEST(sparse_matrix_multiplication_mpi_tbb, test_square_matrices) {
  ...
  sparse_matrix_multiplication_mpi_tbb::CCSMatrixMpiTbb multiplicationTask(taskData);
  ...
  if (multiplicationTask.GetWorldRank() == 0) {
    for (size_t i = 0; i < result.size(); i++)
      EXPECT_NEAR(result[i], expectedOutput[i], epsilon) << "Mismatch at index " << i;
  }
}

TEST(sparse_matrix_multiplication_mpi_tbb, test_rectangular_matrices) {
  const auto epsilon = 1e-6;
...
TEST(sparse_matrix_multiplication_mpi_tbb, test_acceptable_sizes) {
  std::vector<double> matrixA{0, 1, 0, 6, 0, 0, 4, 3, 1, 0, 0, 2};
  std::vector<double> matrixB{0.5, 0, 1.5, 0, 0, 8.0, 3.0, 0, 0, 7, 0, 2};
...
TEST(sparse_matrix_multiplication_mpi_tbb, test_empty_matrices_multiplication) {
...
TEST(sparse_matrix_multiplication_mpi_tbb, test_random_small_square_matrices_multiplication) {
  const auto epsilon = 1e-6;
  const auto size = 4;

  auto matrixA = sparse_matrix_multiplication_mpi_tbb::GenerateRandomMatrix(size * size);
  auto matrixB = sparse_matrix_multiplication_mpi_tbb::GenerateRandomMatrix(size * size);
  ...
  auto expectedOutput =
      sparse_matrix_multiplication_mpi_tbb::MultiplyMatrices(matrixA, size, size, matrixB, size, size);
...
TEST(sparse_matrix_multiplication_mpi_tbb, test_random_large_square_matrices_multiplication) {
  const auto epsilon = 1e-6;
  const auto size = 50;
...
TEST(sparse_matrix_multiplication_mpi_tbb, test_random_large_matrices_multiplication) {
  const auto epsilon = 1e-6;

  auto matrixA = sparse_matrix_multiplication_mpi_tbb::GenerateRandomMatrix(240);
  auto matrixB = sparse_matrix_multiplication_mpi_tbb::GenerateRandomMatrix(240);
  ...
  auto expectedOutput = sparse_matrix_multiplication_mpi_tbb::MultiplyMatrices(matrixA, 12, 20, matrixB, 20, 12);
...
TEST(sparse_matrix_multiplication_mpi_tbb, test_matrices_200) {
  const auto size = 200;
  ...
  sparse_matrix_multiplication_mpi_tbb::CCSMatrixMpiTbb multiplicationTask(task_data_omp);
  ASSERT_TRUE(multiplicationTask.Validation()) << "Validation failed for random large matrices!";
...
TEST(sparse_matrix_multiplication_mpi_tbb, test_matrices_300) {
  const auto size = 300;
  ...
  multiplicationTask.Run();
  clock_t end = clock();
  if (multiplicationTask.GetWorldRank() == 0) std::cout << "Time on matrix 300*300 = " << double(end - start);
  multiplicationTask.PostProcessing();
}

TEST(sparse_matrix_multiplication_mpi_tbb, test_matrices_400) {
  const auto size = 400;
  ...
  multiplicationTask.Run();
  clock_t end = clock();
  if (multiplicationTask.GetWorldRank() == 0) std::cout << "Time on matrix 400*400 = " << double(end - start);
  multiplicationTask.PostProcessing();
}