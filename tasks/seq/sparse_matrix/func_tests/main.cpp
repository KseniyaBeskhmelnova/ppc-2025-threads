...
TEST(sparse_matrix_multiplication_seq, test_square_matrices) {
  ...
  sparse_matrix_multiplication_seq::CCSMatrixSeq multiplicationTask(taskData);
...
TEST(sparse_matrix_multiplication_seq, test_rectangular_matrices) {
  const auto epsilon = 1e-6;
...
TEST(sparse_matrix_multiplication_seq, test_acceptable_sizes) {
  std::vector<double> matrixA{0, 1, 0, 6, 0, 0, 4, 3, 1, 0, 0, 2};
  std::vector<double> matrixB{0.5, 0, 1.5, 0, 0, 8.0, 3.0, 0, 0, 7, 0, 2};
...
TEST(sparse_matrix_multiplication_seq, test_empty_matrices_multiplication) {
...
TEST(sparse_matrix_multiplication_seq, test_usual_multiplication_matrices) {
  const auto epsilon = 1e-6;
  ...
  auto result = sparse_matrix_multiplication_seq::MultiplyMatrices(matrixA, 3, 4, matrixB, 4, 3);

  for (size_t i = 0; i < result.size(); i++)
    EXPECT_NEAR(result[i], expectedOutput[i], epsilon) << "Mismatch at index " << i;
}

TEST(sparse_matrix_multiplication_seq, test_random_large_matrices_multiplication) {
  const auto epsilon = 1e-6;

  auto matrixA = sparse_matrix_multiplication_seq::GenerateRandomMatrix(240);
  auto matrixB = sparse_matrix_multiplication_seq::GenerateRandomMatrix(240);
  ...
  auto expectedOutput = sparse_matrix_multiplication_seq::MultiplyMatrices(matrixA, 12, 20, matrixB, 20, 12);
...
TEST(sparse_matrix_multiplication_seq, test_random_square_matrices_multiplication) {
  const auto epsilon = 1e-6;
  const auto size = 50;
  ...
  auto expectedOutput =
      sparse_matrix_multiplication_seq::MultiplyMatrices(matrixA, size, size, matrixB, size, size);
...
TEST(sparse_matrix_multiplication_seq, test_matrices_200) {
  const auto size = 200;
...
TEST(sparse_matrix_multiplication_seq, test_matrices_300) {
  const auto size = 300;
