...
TEST(sparse_matrix_multiplication_tbb, test_pipeline_run) {
  ...
  auto test_task_sequential = std::make_shared<sparse_matrix_multiplication_tbb::CCSMatrixTBB>(task_data_seq);
...
TEST(sparse_matrix_multiplication_tbb, test_task_run) {
  const auto epsilon = 1e-6;
  const auto size = 300;
